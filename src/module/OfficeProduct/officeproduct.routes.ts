...
router.get("/specific_office_selling_infastructute/:officecategorieId",auth(USER_ROLE.EMPLOYEE,USER_ROLE.ADMIN,USER_ROLE.USER),OfficeProductController.Find_Specific_Selling_Office_Infastructure);
...
router.patch("/update_specific_office_infastructure/:id",auth(USER_ROLE.ADMIN),validationRequest(OfficeProductValidation.TUpdateOfficeInfrastructureSchema),OfficeProductController.UpdateOffice_Infastructure);
...
router.delete("/delete_office_product/:officeproductId",auth(USER_ROLE.ADMIN),OfficeProductController. DeleteOfficeProduct);
